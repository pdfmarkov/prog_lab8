...
public class CommandReader {
	...
	public CommandReader(Sender sender) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, CommandAlreadyExistsException, ClassNotFoundException {
		this.sender = sender;
	...
	private void readCommand(String line, Scanner scanner) throws NotFoundCommandException{
		...
		if (name.equals("exit")) {
			...
			user = (User) args[0];
			sender.send(new main.answers.Request(user, command, args));
		} else {
			Object[] args = CommandManager.parseArgs(line);
			Command command = CommandManager.getCommand(name);
			Object[] fillableArg = CommandManager.getFillableArgs(command, scanner);
